...
const RESUME_EXCERPT_MAX_CHARS = 12000;

/**
 * 生成用于面试的简历摘要（结构化分析 + 可选原文摘录，供面试官系统提示）
 */
export function generateResumePrompt(
  ...
  if (isEn) {
    ...
    lines.push('- Do not read the resume aloud or quote long passages; use it only to steer questions');
  } else {
    ...
    lines.push('- 不要大段背诵简历原文，仅用于引导追问方向');
  }

  const excerpt = (rawResumeText ?? '').trim();
  if (excerpt.length > 0) {
    const clipped =
      excerpt.length > RESUME_EXCERPT_MAX_CHARS
        ? `${excerpt.slice(0, RESUME_EXCERPT_MAX_CHARS)}\n\n${isEn ? '…(resume text truncated)' : '…（简历原文已截断，前 ' + String(RESUME_EXCERPT_MAX_CHARS) + ' 字）'}`
        : excerpt;
    lines.push('');
    if (isEn) {
      lines.push('## Resume text excerpt (for detail; do not recite to candidate)');
    } else {
      lines.push('## 简历原文摘录（供核对细节与追问，勿向候选人逐字复述）');
    }
    lines.push(clipped);
  }
